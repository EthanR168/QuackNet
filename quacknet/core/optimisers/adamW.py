...
class AdamW:
    def __init__(self, forwardPropagationFunction, backwardPropagationFunction, giveInputsToBackprop = False, weightDecay = 0.01):
        self.firstMoment = {}
        self.secondMoment = {}
        self.t = 0
    ...
    def optimiser(self, inputData, labels, useBatches, batchSize, alpha, beta1=0.9, beta2=0.999, epsilon=1e-8):
        if(useBatches == True):
            return self._AdamWOptimiserWithBatches(inputData, labels, batchSize, alpha, beta1, beta2, epsilon)
        else:
            return self._AdamWOptimiserWithoutBatches(inputData, labels, alpha, beta1, beta2, epsilon)

    def _AdamWOptimiserWithBatches(self, inputData, labels, batchSize, alpha, beta1, beta2, epsilon):     
        AllOutputs = []
        ...
        for i in range(numBatches):
            ...
            Parameters = self._AdamW(Parameters, Gradients, alpha, beta1, beta2, epsilon)
        return AllOutputs, Parameters

    def _AdamWOptimiserWithoutBatches(self, inputData, labels, alpha, beta1, beta2, epsilon):   
        AllOutputs = []
    ...
    def _AdamW(self, Parameters, Gradients, alpha, beta1, beta2, epsilon):
        self.t += 1
        for key in Gradients:
            if(isinstance(Gradients[key], list)): # if the Gradient is a inhomengous list (jagged array, which numpy doesnt like)
                for i, grad in enumerate(Gradients[key]):
                    ...
                    Parameters[key][i] -= alpha * self.weightDecay * Parameters[key][i]
            else:
                if key not in self.firstMoment:
                ...
                Parameters[key] -= alpha * self.weightDecay * Parameters[key]
        return Parameters
